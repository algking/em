...
if $GNU_TAR ; then
    JDE_DIR_TMP=`$UNPACK_CMD $JDE_FILE.$FILE_SUFFIX | tail -n 1`
    ELIB_DIR_TMP=`$UNPACK_CMD $ELIB_FILE.$FILE_SUFFIX | tail -n 1`
    CEDET_DIR_TMP=`$UNPACK_CMD $CEDET_FILE.$FILE_SUFFIX | tail -n 1`

    JDE_DIR=`echo $JDE_DIR_TMP | sed -e "s%^\([^/]*\)/.*%\1%"`
    ELIB_DIR=`echo $ELIB_DIR_TMP | sed -e "s%^\([^/]*\)/.*%\1%"`
    CEDET_DIR=`echo $CEDET_DIR_TMP | sed -e "s%^\([^/]*\)/.*%\1%"`
else
    JDE_DIR_TMP=`$UNPACK_CMD $JDE_FILE.$FILE_SUFFIX 2>&1 | tail -n 1`
    ELIB_DIR_TMP=`$UNPACK_CMD $ELIB_FILE.$FILE_SUFFIX 2>&1 | tail -n 1`
    CEDET_DIR_TMP=`$UNPACK_CMD $CEDET_FILE.$FILE_SUFFIX 2>&1 | tail -n 1`

    JDE_DIR=`echo $JDE_DIR_TMP | sed -e "s%^\([^/]*\)/.*%\1%" -e "s/^x *//"`
    ELIB_DIR=`echo $ELIB_DIR_TMP | sed -e "s%^\([^/]*\)/.*%\1%" -e "s/^x *//"`
